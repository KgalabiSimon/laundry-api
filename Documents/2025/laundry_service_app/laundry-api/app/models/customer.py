...
class Customer(Base):
    ...
    # Relationships (explicit join/FK for clarity)
    user = relationship(
        "User",
    ...
    def __repr__(self):
        tier = getattr(self.loyalty_tier, "value", self.loyalty_tier)
        return f"<Customer(id={self.id}, user_id={self.user_id}, tier='{tier}')>"

...
class Worker(Base):
    ...
    user = relationship(
        ...
        primaryjoin="Worker.user_id == User.id",
    )
    created_by = relationship(
        "User",
        foreign_keys=[created_by_id],
        back_populates="workers_created",
    )

    def __repr__(self):
        role = getattr(self.worker_role, "value", self.worker_role)
        return f"<Worker(id={self.id}, employee_id='{self.employee_id}', role='{role}')>"
